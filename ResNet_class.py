...
class MixLinear(nn.Linear):
    ...
    def forward(self, x: torch.Tensor, targets = None, mode='softmax_threshold', fixed = False, alpha=0.8, is_detach = True, temperature = 1.,is_normalized = False):
        ...
        if self.training and mode=='fedosr':
            ...
            prob = torch.nn.functional.softmax(outputs.detach()/temperature, dim=-1)
            fixed = fixed
            alpha = alpha
            if is_detach:
            ...
            for i in range(x.shape[0]):
                ...
                mix_proto_instance = torch.stack(mix_proto_instance, dim = 0)
                mix_proto_batch.append(mix_proto_instance)
            mix_proto_batch = torch.stack(mix_proto_batch, dim = 0)
            if is_normalized:
                mix_out = torch.bmm(mix_proto_batch, x.unsqueeze(-1)).squeeze(-1) * 1.0/(weight.shape[0]-1)
            else:
                mix_out = torch.bmm(mix_proto_batch, x.unsqueeze(-1)).squeeze(-1)          
            outputs = torch.cat([outputs, mix_out], dim=1)
...
def resnet18(pretrained=False, num_classes=8, **kwargs):
    ...
    return _resnet(BasicBlock, [2, 2, 2, 2], pretrained, num_classes=num_classes,
                   **kwargs)


def resnet34(pretrained=False, num_classes=8, **kwargs):

    return _resnet(BasicBlock, [3, 4, 6, 3], pretrained, num_classes=num_classes,
                   **kwargs)


def resnet50(pretrained=False, num_classes=8, **kwargs):

    return _resnet(Bottleneck, [3, 4, 6, 3], pretrained, num_classes=num_classes,
                   **kwargs)
